...
RSpec.feature "Evidence", type: :system do
  ...
  def then_i_see_uploaded_evidence_form_validation_errors
    expect(page).to have_content("Select yes if you have more evidence to upload")
  end
  ...
  def then_i_am_asked_to_confirm_the_updated_evidence_details
    ...
    expect_summary_row(
      ...
      value: "Yes",
      change_link: edit_referral_allegation_evidence_start_path(@referral, return_to: current_path)
    )
    ...
    expect_summary_row(
      ...
      value: "upload2.pdf\nupload.txt",
      change_link:
        edit_referral_allegation_evidence_uploaded_path(@referral, return_to: current_path)
    )
  end
  ...
  def and_i_visit_uploaded_files
    visit edit_referral_allegation_evidence_uploaded_path(@referral)
  end
