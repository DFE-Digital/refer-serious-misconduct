...
RSpec.describe Referrals::TeacherRole::StartDateForm, type: :model do
  subject(:form) { described_class.new(date_params:, referral:, role_start_date_known:) }
  ...
  describe "#valid?" do
    ...
    context "when role_start_date_known is blank" do
      ...
      it "adds an error" do
        expect(form.errors[:role_start_date_known]).to eq(["Select yes if you know when they started the job"])
      end
    end
  end
  ...
  describe "#save" do
    ...
    context "when role_start_date_known is false" do
      ...
      it "updates the start_date_known to false" do
        expect { form.save }.to change(referral, :role_start_date_known).from(nil).to(false)
      end
    end
  end
end
