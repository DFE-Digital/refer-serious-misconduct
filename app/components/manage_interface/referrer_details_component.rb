module ManageInterface
  class ReferrerDetailsComponent < ViewComponent::Base
    ...
    def rows
      ...
      rows.push({ key: { text: "Job title" }, value: { text: referrer.job_title } }) if referral.from_employer?
      ...
      if referral.from_employer?
        rows.push({ key: { text: "Employer" }, value: { text: organisation_address(referral.organisation) } })
      end
