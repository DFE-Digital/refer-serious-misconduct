class LogStashFormatter < SemanticLogger::Formatters::Raw
  ...
  def format_backtrace
    ...
    # If the payload also has a :stack_trace key, trim it
    # and pass only the first line of the stack trace, under the backtrace key
    # to avoid passing an object that is too big, which causes Logstash to fail.
    stack_trace = hash.dig(:exception, :stack_trace)

    if stack_trace.present?
      hash[:backtrace] = "#{hash[:backtrace]}\n#{stack_trace.first}"
...
if ENV["LOGSTASH_HOST"] && ENV["LOGSTASH_PORT"]
  ...
  log_stash =
    LogStashLogger.new({ host: ENV["LOGSTASH_HOST"], port: ENV["LOGSTASH_PORT"] }.merge(customize_event: fix_payload))
